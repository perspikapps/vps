#!/usr/bin/env bats
# Static checks for vps-summary/ - not an installable step (excluded from
# dispatch.sh's feature discovery), so no up()/down()/vps.order to check.

REPO_ROOT="$(cd "$(dirname "${BATS_TEST_FILENAME}")/.." && pwd)"
DIR="$REPO_ROOT/vps-summary"
...
@test "package.json bin entry matches the folder name" {
    run node -e "const p = require('$DIR/package.json'); process.exit(p.bin['vps-summary'] === 'run.sh' ? 0 : 1)"
    [ "$status" -eq 0 ]
}
