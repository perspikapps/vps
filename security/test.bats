#!/usr/bin/env bats
# Static checks for security/run.sh and package.json - a live install
...
@test "package.json bin entry matches the folder name" {
    run node -e "const p = require('$DIR/package.json'); process.exit(p.bin['security'] === 'run.sh' ? 0 : 1)"
    [ "$status" -eq 0 ]
}
